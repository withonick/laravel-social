            ...
            <tbody>
                @for($i = 0; $i<count($users); $i++)
                    <tr>
                        ...
                        <td><span class="name">{{count($users[$i]->whosubscribed)}}</span></td>
                        ...
                        <td>
                            ...
                            @else
                                <form action="{{route('user.unban', $users[$i]->id)}}" method="post">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="border-0 bg-transparent" style="cursor:pointer;"><span class="badge badge-pending">Inactive</span></button>
                                </form>
                            @endif
                        </td>
                    </tr>
